...
def check3_latent_only(new_mod, arch):
    model = build_unified(new_mod, SMALL, arch, seed=2)
    model.eval()
    batch = make_batch(SMALL)
    with torch.no_grad():
        g, t, h = model.encode(batch["action"], batch["x0_feat"], batch["x1_feat"])
        act_joint, _ = model.unified_decoder(g, t, h, batch["x0_feat"])
        act_lat_only = model.decode(g, t, h)  # no image
        x0_noise = torch.randn_like(batch["x0_feat"]) * 100.0
        act_noise, _ = model.unified_decoder(g, t, h, x0_noise)
