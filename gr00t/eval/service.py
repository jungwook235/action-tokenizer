    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                import traceback
                error_stack = traceback.format_exc() # 에러의 전체 경로(traceback) 추출
                print(error_stack)
                
                # b"ERROR" 대신, 에러의 종류와 메시지를 합쳐서 보냄
                error_response = f"ERROR_DETAILS:{type(e).__name__}: {str(e)}"
...
class BaseInferenceClient:
    ...
    def call_endpoint(
        ...
        message = self.socket.recv()
        #if message == b"ERROR":
        #    raise RuntimeError(f"Server Side Exception: {message.decode(errors='ignore')}")
        # === 수정 부분: 서버가 보낸 메시지가 에러인지 먼저 확인 ===
        if message.startswith(b"ERROR"):
            # 에러 메시지를 텍스트로 읽어서 예외를 발생시킵니다.
            error_msg = message.decode('utf-8', errors='ignore')
            raise RuntimeError(f"Server Side Exception: {error_msg}")
        # ======================================================
