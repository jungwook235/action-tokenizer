...
from .embodiment_tags import EmbodimentTag, get_name_from_value
from .schema import (
    DatasetMetadata,
...
try:
    from gr00t.data.rlds.oxe import OXE_NAMED_MIXTURES, get_oxe_dataset_kwargs_and_weights
    from gr00t.data.rlds.oxe.configs import OXE_DATASET_CONFIGS, oxe_state_scheme_to_keys, oxe_action_scheme_to_keys
    from gr00t.data.rlds import make_gr00t_interleaved_rlds_dataset
except Exception as e:
    _print(f"[w] Failed to import RLDS OXE modules. Please ensure that the RLDS package is installed. {e}")
    pass
...
def calculate_dataset_statistics(parquet_paths: list[Path]) -> dict:
    ...
    for parquet_path in tqdm(
        ...
        parquet_data = parquet_data.drop(
            columns=["observation.tactile.left", "observation.tactile.right"],
            errors='ignore'  # 해당 열이 없는 경우 에러를 무시합니다.
        )
    ...
    for le_modality in all_low_dim_data.columns:
        ...
        # if "tactile" in le_modality:
        #     print(f"Skipping {le_modality} because it is a tactile")
        #     continue
...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        if not modality_meta_path.exists() and str(self.dataset_path).endswith("open_airfryer_v1_lerobot_eef_resized"):
            modality_meta_path = Path(
                "/sjw_alinlab1/home/jungwook/Isaac-GR00T/sbatch_scripts/eccv/ablation/combined/franka/modality.json"
            )
        #modality_meta_path = Path(ABS_TASK_INDEX_CUSTOM_LE_ROBOT_MODALITY_FILENAME)
        assert (
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        #print("Length of all_steps", len(self.all_steps))
        #BridgeV2: Length of all_steps 1893026
        #print("Index", index)
        #print("Trajectory ID", self.all_steps[index][0])
        #print("Base Index", self.all_steps[index][1])
        # exit()
        """print(f"\n[알림] 최초 호출 확인: 전체 데이터를 순회하며 액션을 추출합니다...")
            
        collected_actions = []
        
        # 속도 향상을 위해 비디오 로딩 잠시 차단
        #original_video_keys = self.modality_keys.get('video', []).copy()
        #self.modality_keys['video'] = []
        for i in tqdm(range(len(self.all_steps)), desc="전체 액션 추출 중"):
            traj_id, b_idx = self.all_steps[i]
            
            # 2. 본래 getitem이 하던 데이터 추출 및 변환 로직 그대로 수행
            raw_step = self.get_step_data(traj_id, b_idx)
            processed = self.transforms(raw_step)
            
            action = processed.get('action')
            mask = processed.get('action_mask')
            
            # 3. FAST fit에 유용한 형태(마스크가 모두 True인 청크)만 필터링
            if action is not None and mask is not None:
                is_valid = False
                if isinstance(mask, np.ndarray):
                    is_valid = mask.all()
                else:
                    is_valid = torch.all(mask).item()

                if is_valid:
                    act_np = action.detach().cpu().numpy() if torch.is_tensor(action) else action
                    collected_actions.append(act_np.copy())
        final_array = np.stack(collected_actions)
        save_name = "fast_action_data.npy"
        np.save(save_name, final_array)
        print(f"\n[완료] {len(collected_actions)}개의 액션이 {save_name}에 저장되었습니다.")
        print(final_array.shape,final_array[0])
        exit()"""

        trajectory_id, base_index = self.all_steps[index]
        ret_data = self.transforms(self.get_step_data(trajectory_id, base_index))

        """[Dataset] ret_data keys: ['state', 'state_mask', 'segmentation_target', 'segmentation_target_mask', 'has_real_action', 'action', 'action_mask', 'eagle_content', 'embodiment_id']
  state: shape=(2, 64), dtype=float64
  state_mask: shape=(2, 64), dtype=bool
  segmentation_target: shape=(2,), dtype=float64
  segmentation_target_mask: shape=(1,), dtype=float64
  has_real_action: shape=(), dtype=bool
  action: shape=(16, 32), dtype=float64
  action_mask: shape=(16, 32), dtype=bool
  eagle_content: type=<class 'dict'>
  eagle_content keys: ['image_inputs', 'video_inputs', 'text_list']"""
        return ret_data
    ...
    def get_step_data(self, trajectory_id: int, base_index: int) -> dict:
        ...
        #    if "video" in k and v.shape[0] != 2:
        #        rand_traj_id = np.random.randint(0, len(self.trajectory_ids))
        #        print("Random Trajectory sampled, original traj {}, new traj {} - LeRobotSingleDataset".format(trajectory_id, rand_traj_id))
        #        return self.get_step_data(rand_traj_id, 0)
    ...
    def get_language(
        ...
        #print(f"1. Original key for annotation {subkey} is {original_key}", flush=True) #Original key for annotation human.action.task_description is task_index
        if original_key is None:
            original_key = key
    ...
    def merge_metadata(
        ...
        for metadata in metadata_dicts:
            for modality, configs in metadata["modalities"].items():
                serialized_config = json.dumps(configs, sort_keys=True)
                if serialized_config not in modality_configs[modality]:
                    modality_configs[modality].append(serialized_config)
        merged_metadata["modalities"] = {}
        for modality, configs in modality_configs.items():
            if len(configs) > 1:
                _print(
                    f"[w] Multiple modality configs for modality {modality}: {configs}. "
                    f"Using the first config (index 0)."
                )
            merged_metadata["modalities"][modality] = json.loads(configs[0])
...
class RLDSDataset(IterableDataset):
    def __init__(
        ...
        future_action_window_size: int = 15, # TY: 16 - window_size
        ...
        if not Path(data_root_dir).exists():
            raise FileNotFoundError(f"Dataset path {data_root_dir} does not exist")
        ...
        per_dataset_kwargs, weights = get_oxe_dataset_kwargs_and_weights(
            self.data_root_dir,
            mixture_spec,
            load_camera_views=("primary", "secondary", "wrist"), # type: ignore 
            load_depth=False,
            load_proprio=True,
            load_language=True,
        )
        ...
        brightness = 0.3
        contrast = max(0.0, 1.0 - 0.4) , 1.0 + 0.4
        saturation = max(0.0, 1.0 - 0.4) , 1.0 + 0.4
        hue = 0.08
        ...
        rlds_config = dict(
            ...
            frame_transform_kwargs=dict(
                resize_size=resize_resolution if resize_resolution is not None else {},
                num_parallel_calls=-1, # For CPU-intensive ops (decoding, resizing, etc.) NOTE(TY): Fixed (was 16, seems like AUTOTUNE accelerates a little bit)
                image_augment_kwargs=dict(
                    random_resized_crop=dict(scale=[0.95, 0.95], ratio=[1.0, 1.0]),
                    random_brightness=[brightness],
                    random_contrast=[contrast[0], contrast[1]], # type: ignore
                    random_saturation=[saturation[0], saturation[1]], # type: ignore
                    random_hue=[hue], # type: ignore
                    augment_order=[
                        "random_resized_crop", # crop and resize again
                        "random_brightness",
                        "random_contrast",
                        "random_saturation",
                        "random_hue",
            ...
            traj_transform_threads=len(mixture_spec), #NOTE(TY): autotuning this leads to OOM
            traj_read_threads=len(mixture_spec), #NOTE(TY): autotuning this leads to OOM
            train=train,
        )
        ...
        self.all_transforms = dict()
        self.merged_metadata = dict()

        # NOTE: import here to avoid circular import with gr00t.experiment.data_config,
        # which itself needs ModalityConfig from this module.
        from gr00t.experiment.data_config import load_data_config

        for dataset_name, weight in mixture_spec:
            config_name = dataset_name
            if "agibot_gripper" in dataset_name:
                config_name = "agibot_gripper"
            elif "galaxea" in dataset_name:
                config_name = "galaxea"
            elif "neural_gr1" in dataset_name:
                config_name = "neural_gr1"
            data_config_cls = load_data_config(config_name, from_oxe=True)
            transforms = data_config_cls.transform(backbone_model_type=backbone_model_type, backbone_path=backbone_path)
            metadata = self._get_metadata_from_statistics(dataset_name)
            transforms.set_metadata(metadata)
            self.all_transforms[config_name] = transforms # 1 transform for all parts of the same dataset
            self.merged_metadata[dataset_name] = metadata

    def _get_metadata_from_statistics(self, dataset_name: str) -> DatasetMetadata:
        """Get the metadata for the dataset from the statistics."""
        rlds_statistics = self.dataset_statistics[dataset_name]
        config_name = dataset_name
        if "agibot_gripper" in dataset_name:
            config_name = "agibot_gripper"
        elif "galaxea" in dataset_name:
        ...
        oxe_config = OXE_DATASET_CONFIGS[config_name]
        state_encoding = oxe_config["state_encoding"]
        action_encoding = oxe_config["action_encoding"]
        state_keys = oxe_state_scheme_to_keys(state_encoding)
        action_keys = oxe_action_scheme_to_keys(action_encoding)
        state_statistics = dict()
        state_modalities = dict()
        action_statistics = dict()
        action_modalities = dict()
        for key, start_idx, end_idx, state_action_metadata in state_keys:
            if key is None:
                continue
            rlds_state_stats = rlds_statistics["proprio"]
            state_statistics[key] = DatasetStatisticalValues(
                max=rlds_state_stats["max"][start_idx:end_idx],
                min=rlds_state_stats["min"][start_idx:end_idx],
                mean=rlds_state_stats["mean"][start_idx:end_idx],
                std=rlds_state_stats["std"][start_idx:end_idx],
                q01=rlds_state_stats["q01"][start_idx:end_idx],
                q99=rlds_state_stats["q99"][start_idx:end_idx],
            )
            state_modalities[key] = state_action_metadata

        for key, start_idx, end_idx, state_action_metadata in action_keys:
            rlds_action_stats = rlds_statistics["action"]
            action_statistics[key] = DatasetStatisticalValues(
                max=rlds_action_stats["max"][start_idx:end_idx],
                min=rlds_action_stats["min"][start_idx:end_idx],
                mean=rlds_action_stats["mean"][start_idx:end_idx],
                std=rlds_action_stats["std"][start_idx:end_idx],
                q01=rlds_action_stats["q01"][start_idx:end_idx],
                q99=rlds_action_stats["q99"][start_idx:end_idx],
            )
            action_modalities[key] = state_action_metadata
        ...
        embodiment_tag=EmbodimentTag(config_name)
    ...
    def make_dataset(self, rlds_config) -> Tuple[tf.data.Dataset, int, dict]:
        return make_gr00t_interleaved_rlds_dataset(**rlds_config)

    # === NOTE(TY): This is used ===
    def __iter__(self):
        ...
        # total_shards = ddp_world_size
        # shard_id = ddp_rank
        # sharded_dataset = self.dataset.shard(num_shards=total_shards, index=shard_id)

        # for rlds_batch in sharded_dataset.as_numpy_iterator():
        #     yield self.get_data(rlds_batch)

        # NOTE(TY): No need of sharding. The sharded dataset files (TFRecords) are shuffled before they are loaded
        for rlds_batch in self.dataset.as_numpy_iterator():
            yield self.get_data(rlds_batch)

    def get_data(self, rlds_batch: dict) -> dict:
        """Match the data loaded from RLDS to the schema."""
        embodiment_tag = rlds_batch.pop("embodiment_tag")
        control_frequency = rlds_batch.pop("control_frequency", None)
        dataset_name = get_name_from_value(embodiment_tag)
        oxe_config = OXE_DATASET_CONFIGS[dataset_name]
        state_encoding = oxe_config["state_encoding"]
        action_encoding = oxe_config["action_encoding"]
        data_batch = dict()

        state_keys, start_idxs, end_idxs, _ = zip(*oxe_state_scheme_to_keys(state_encoding))
        if state_keys[0] is not None:
            for i, key in enumerate(state_keys):
                data_batch[f"state.{key}"] = rlds_batch["observation"]["proprio"][..., start_idxs[i]:end_idxs[i]]

        action_keys, start_idxs, end_idxs, _ = zip(*oxe_action_scheme_to_keys(action_encoding))
        for i, key in enumerate(action_keys):
            data_batch[f"action.{key}"] = rlds_batch["action"][..., start_idxs[i]:end_idxs[i]]

        data_batch.update({
            "video.image_primary": rlds_batch["observation"]["image_primary"],
            "video.image_secondary": rlds_batch["observation"]["image_secondary"],
            "video.image_wrist": rlds_batch["observation"]["image_wrist"],
            "annotation.task_description": [rlds_batch["task"]["language_instruction"].decode("utf-8")],
        })

        if dataset_name=="galaxea": # Galaxea instruction format: format: "high level"@"low level chinese"@"low level english"
            data_batch["annotation.task_description"] = [data_batch["annotation.task_description"][0].split("@")[-1]]
        # _print(f"[DEBUG] Dataset: {dataset_name}, task_description: {data_batch['annotation.task_description']}")

        # Add control_frequency to data_batch if present
        if control_frequency is not None:
            # Extract scalar value from tensor (assuming it's constant across trajectory)
            if hasattr(control_frequency, 'numpy'):
                control_freq_value = float(control_frequency.numpy()[0] if control_frequency.shape else control_frequency.numpy())
            else:
                control_freq_value = float(control_frequency[0] if len(control_frequency.shape) > 0 else control_frequency)
            data_batch["control_frequency"] = control_freq_value

        # Add dataset_name to data_batch for ContextVLA chat template (RLDS datasets only)
        # We need to repeat it for the entire batch so that transforms.apply_batch can split it correctly
        batch_size = next(iter(data_batch.values())).shape[0]
        data_batch["dataset_name"] = [dataset_name] * batch_size

        transforms = self.all_transforms[dataset_name]
        data_batch = transforms(data_batch)
