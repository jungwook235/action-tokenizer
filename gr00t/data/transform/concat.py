...
class AnyResolutionConcatTransform(ConcatTransform):
    """
    Concatenate the keys according to specified order.
    But for Images, we just gather them into a list, because they might have different resolutions.
    """
    def apply(self, data: dict) -> dict:
        grouped_keys = {}
        for key in data.keys():
        ...
        if "video" in grouped_keys:
            ...
            # Process each video view
            # Collect into List[np.ndarray(T, H, W, C)]
            videos = []
            for video_key in self.video_concat_order:
                video = data.pop(video_key)
                # Check if it's a padding image (0x0)
                # Assuming shape is [T, H, W, C]
                # If H==0 and W==0, then we skip it.
                if len(video.shape) >= 3 and (video.shape[-3] == 0 or video.shape[-2] == 0):
                    continue
                videos.append(video)
