...
class ActionStateDataset(ActionOnlyDataset):
    ...
    def _build_action_state_transforms(
        ...
        data_config 에 정의된 per-key normalization_modes 와 state_target_rotations 를
        그대로 respect 하므로 tokenizer 학습 시 VLA 와 동일한 action/state 표현을
        학습 타겟으로 삼을 수 있다.
        """
        transform_list = []

        # Action transforms (rotation 변환은 action 에 적용 안 함 — 기존 관례 유지)
...
class PreTransformedActionStateDataset(Dataset):
    ...
    def __init__(
        ...
        # data_config 가 state_target_rotations (quat→rotation_6d) 를 respect 하도록 바뀌면
        ...
        # --- Main caching loop ---
        for i in tqdm(range(n), desc=f"[PreTransform][{split}] Caching action+state"):
            if i == 0:
                sample = first
            else:
                sample = source[i]
        ...
        # --- Future state caching (trajectory-level access 필요) ---
        if future_cache is not None:
            print(f"[PreTransform][{split}] Caching future hand states...")
            # trajectory 정보에 접근하여 미래 state 가져오기
            for i in tqdm(range(n), desc=f"[PreTransform][{split}] Future states"):
                traj_id, base_idx = source.all_steps[i]
                traj_index = source.get_trajectory_index(traj_id)
                traj_len = source.trajectory_lengths[traj_index]

                for fi, step_offset in enumerate(hand_pred_future_steps):
                    future_idx = min(base_idx + step_offset, traj_len - 1)
                    # future step의 데이터를 가져와서 transform 적용
