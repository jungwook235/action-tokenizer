...
from gr00t.data.rlds.constants import NormalizationType
from gr00t.data.rlds import obs_transforms, traj_transforms
from gr00t.data.rlds.utils import goal_relabeling, task_augmentation
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    do_normalize: Optional[bool] = True, #TY: Added to optionally skip normalization
    control_frequency: Optional[float] = None,  # Control frequency for frequency-scaling RoPE
    ...
    def restructure(traj):
        ...
        if "agibot_gripper" in embodiment_name:
            embodiment_name = "agibot_gripper"
        elif "galaxea" in embodiment_name:
            embodiment_name = "galaxea"
        elif "neural_gr1" in embodiment_name:
            embodiment_name = "neural_gr1"

        traj = {
            "observation": new_obs,
            "task": task,
            "action": tf.cast(traj["action"], tf.float32),
            "embodiment_tag": EMBODIMENT_TAG_MAPPING[embodiment_name], #TY: Added embodiment tag, to find the proper transform later
            # "dataset_name": tf.repeat(name, traj_len),
        }

        # Add control_frequency to trajectory if provided (for frequency-scaling RoPE)
        if control_frequency is not None:
            traj["control_frequency"] = tf.repeat(tf.constant(control_frequency, dtype=tf.float32), traj_len)
    ...
    _print(f"[DEBUG] name: {name}")
    if name == "dobbe":
        builder = tfds.builder(name, data_dir=data_dir, version="0.0.1")
    else:
    ...
    if isinstance(dataset_statistics, str):
        ...
        logging.info("-" * 30) # TY: For debug, print dataset name and version
        logging.info(f"Dataset Name: {builder.name}")
        logging.info(f"Version: {builder.version}")

        full_dataset = dl.DLataset.from_rlds(
            builder, split="all", shuffle=False, num_parallel_reads=num_parallel_reads
        ).traj_map(restructure, num_parallel_calls)
        # tries to load from cache, otherwise computes on the fly
        _print("[DEBUG] Getting statistics for dataset: ", builder.name)
        ...
        _print("[DEBUG] Statistics loaded!")
    ...
    if do_normalize:  #TY: Added to optionally skip normalization
...
def make_interleaved_dataset(
    ...
    *,
    ...
    dataset_len = int(sum(sample_weights)) #NOTE(TY): Changed to sum of weights for gr00t
...
def make_gr00t_interleaved_rlds_dataset(  #NOTE(TY): RLDS multiple datasets, gr00t-specific
    dataset_kwargs_list: List[Dict],
    sample_weights: Optional[List[float]] = None,
    *,
    train: bool,
    ...
    dataset_len = int(sum(sample_weights)) # TY: Changed to sum of weights for gr00t
    ...
    #   =>> Note :: Only counting the "primary" datasets (i.e., datasets with sample_weight == 1.0)
    # dataset_len = int((np.array(dataset_sizes) / sample_weights)[primary_dataset_indices].max())
    # dataset_len = int((np.array(dataset_sizes) / sample_weights).max()) # TY: Changed to count all datasets for gr00t
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset, _ = make_dataset_from_rlds(
            **dataset_kwargs,
            ...
            num_parallel_reads=reads,
            dataset_statistics=all_dataset_statistics[dataset_kwargs["name"]],
            do_normalize=False,  #NOTE(TY): Added to optionally skip normalization
        )
        # compute min length in Python and freeze it into the lambda as a TF constant
        min_length = int(traj_transform_kwargs.get("window_size", 1)) \
                     + int(traj_transform_kwargs.get("future_action_window_size", 15))
        min_length_const = tf.constant(min_length, dtype=tf.int32)

        # filter out too-short trajectories BEFORE repeating / transforming
        dataset = dataset.filter(lambda traj, ml=min_length_const: tf.shape(traj["action"])[0] >= ml)
        dataset = apply_trajectory_transforms(
            dataset.repeat(),
            **traj_transform_kwargs,
            num_parallel_calls=threads,
        ...
        # NOTE(TY): Disabled per-dataset frame transforms for gr00t RLDS
        # dataset = apply_per_dataset_frame_transforms(dataset, **dataset_frame_transform_kwargs)
    ...
    # NOTE(TY): Seems to reduce memory usage without affecting speed?
    dataset = dataset.with_ram_budget(1)
    dataset = dataset.prefetch(tf.data.AUTOTUNE)
    dataset.sample_weights = sample_weights  # Save for Later

    return dataset, dataset_len, all_dataset_statistics


def make_gr00t_interleaved_rlds_lerobot_dataset():  #NOTE(TY): TODO: rlds + lerobot mixed datasets
    pass
