...
def pprint_data_mixture(dataset_kwargs_list: List[Dict[str, Any]], dataset_weights: List[int]) -> None:
    _print("\n######################################################################################")
    _print(f"# Loading the following {len(dataset_kwargs_list)} datasets (incl. sampling weight):{'': >24} #")
    for dataset_kwargs, weight in zip(dataset_kwargs_list, dataset_weights):
        pad = 80 - len(dataset_kwargs["name"])
        _print(f"# {dataset_kwargs['name']}: {weight:=>{pad}f} #")
    _print("######################################################################################\n")
...
def get_dataset_statistics(
    ...
    logging.info(f"Dataset statistics hash: {unique_hash}") # TY: For debug, making statistics easier
    ...
    if save_dir is not None:
        if os.path.exists(tf.io.gfile.join(save_dir, f"dataset_statistics_static.json")):
            path = tf.io.gfile.join(save_dir, f"dataset_statistics_static.json")
        else:
    ...
    if tf.io.gfile.exists(path):
        ...
        try:
            ...
            logging.warning(f"Error loading dataset statistics from {path}: {e}. File may be corrupted. Will regenerate.")
            # Try to remove corrupted file and regenerate
            try:
                tf.io.gfile.remove(path)
                logging.info(f"Removed corrupted file {path}. Will regenerate statistics.")
            except Exception as e2:
                logging.warning(f"Could not remove corrupted file {path}: {e2}")
    ...
    if os.path.exists(local_path):
        ...
        try:
            ...
            logging.warning(f"Error loading dataset statistics from {local_path}: {e}. File may be corrupted. Will regenerate.")
            # Try to remove corrupted file and regenerate
            try:
                os.remove(local_path)
                logging.info(f"Removed corrupted file {local_path}. Will regenerate statistics.")
            except Exception as e2:
                logging.warning(f"Could not remove corrupted file {local_path}: {e2}")
