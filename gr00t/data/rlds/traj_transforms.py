...
def chunk_act_obs(traj: Dict, window_size: int, future_action_window_size: int = 0) -> Dict:
    ...
    effective_traj_len = tf.maximum(traj_len - future_action_window_size, 0) # for safety
    tf.debugging.assert_greater_equal(
        traj_len, window_size + future_action_window_size,
        message="trajectory too short for given window sizes"
    )   
    ...
    traj["embodiment_tag"] = tf.fill([effective_traj_len], traj["embodiment_tag"])
