...
class Eagle2_5_VLForConditionalGeneration(Eagle2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        vit_embeds, raw_vit_embeds = self.extract_feature(pixel_values)
        #print(f"vit_embeds shape: {vit_embeds.shape}") 
        #vit_embeds shape: torch.Size([96, 256, 2048])
        #print(f"raw_vit_embeds shape: {raw_vit_embeds.shape}") 
        #raw_vit_embeds shape: torch.Size([96, 256, 1152])
        if image_flags is not None:  ###False 
            image_flags = image_flags.view(-1)
            #num_image_flags = (image_flags == 1).sum().item()
            #print(f"[Eagle2_5_VL] Number of image_flags == 1: {num_image_flags}")
        ...
        #print(f"[Eagle2_5_VL] Number of image tokens (selected=True): {num_selected}") 
        #[Eagle2_5_VL] Number of image tokens (selected=True): 24576 = 32 * 3 * 256
    ...
    def extract_feature(self, pixel_values):
        ...
        return vit_embeds, raw_vit_embeds
