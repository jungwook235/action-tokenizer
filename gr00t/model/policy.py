...
class Gr00tPolicy(BasePolicy):
    ...
    def get_action_with_time_check(self, observations: Dict[str, Any]) -> Dict[str, Any]:
        # let the get_action handles both batch and single input
        is_batch = self._check_state_is_batched(observations)
        if not is_batch:
            observations = unsqueeze_dict_values(observations)
        ...
        normalized_action, time_taken = self._get_action_from_normalized_input_with_time_check(normalized_input)
        unnormalized_action = self._get_unnormalized_action(normalized_action)

        if not is_batch:
            unnormalized_action = squeeze_dict_values(unnormalized_action)
        return unnormalized_action, time_taken
    ...
    def _get_action_from_normalized_input_with_time_check(self, normalized_input: Dict[str, Any]) -> torch.Tensor:
        # Set up autocast context if needed
        with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=COMPUTE_DTYPE):
            model_pred, time_taken = self.model.get_action_with_time_check(normalized_input)

        normalized_action = model_pred["action_pred"].float()
        return normalized_action, time_taken
