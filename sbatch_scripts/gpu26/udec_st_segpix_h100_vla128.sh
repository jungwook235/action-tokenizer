#!/bin/bash
#SBATCH --job-name=gr1_vla128_udec_st_segpix_h100
...
# h100 twin of the h200 job 411 (2026-08-19): background QoS (h100 is not
# wook's guaranteed partition), halved save cadence (5000/2500), distinct
# experiment dirs + run names so nothing collides with 411.
...
TOK_CKPT_DIR=/ckpt/$USER/tok_gr1_v4_udec_st_segpix_h100_bs256
VLA_CKPT_DIR=/ckpt/$USER/vla_gr1_v4_udec_st_segpix_h100_vla128
...
if compgen -G "$TOK_CKPT_DIR/checkpoint-$TOK_STEP/model*" > /dev/null; then
  ...
  python scripts/train_action_latent_tokenizer_v4.py \
    ...
    --run-name "actlat_v4_gr1_udec_st_segpix_h100_1000demos_bs256_vla128" \
    --num-gpus 4 \
    --batch-size 64 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
if compgen -G "$VLA_CKPT_DIR/checkpoint-$VLA_STEP/model*" > /dev/null; then
  ...
  python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v4_gr1_udec_st_segpix_h100_1000demos_bs256_vla128" \
    --num-gpus 4 \
    --batch-size 32 \
    --max-steps $VLA_STEP \
    --save-steps 2500 \
...
echo "[done] udec_st_segpix_h100 pipeline complete"
