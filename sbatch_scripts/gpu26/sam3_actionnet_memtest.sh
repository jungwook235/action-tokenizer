#!/bin/bash
#SBATCH --job-name=an_sam3_memtest
...
OUT_ROOT=/ckpt/$USER/actionnet_sam3_memtest
...
PEAK=/tmp/gpumem_$SLURM_JOB_ID.txt
( while true; do nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits >> "$PEAK"; sleep 2; done ) &
SAMPLER=$!
...
echo "[mem] peak_used_MiB=$(sort -n "$PEAK" | tail -1) of $(nvidia-smi --query-gpu=memory.total --format=csv,noheader,nounits | head -1) MiB"
echo "[mem] samples=$(wc -l < "$PEAK")"
