...
OUT_ROOT=/ckpt/$USER/actionnet_sam3
...
# CLI per analysis/sam3_masking/batch_sam3_actionnet.py (d71bea3): --root is the
# dataset root holding meta/episodes.jsonl + videos/. Per-prompt mask planes are
# --no-prompt-masks (user default): keep union/robot/object planes, drop the
# per-prompt (P,T,H,W) stack — only robot|object are consumed downstream.
# >=4 CPUs per task matters — mp4 encoding is on the CPU inside the 0.27 s/frame.
