...
#
# 2026-08-21, fourth pass on the a100 share of the GR1 remainder. History:
#   job 670  %8, 4 tasks ever ran -> trimmed to tasks 16/17 only
#   job 896  %2, shards 2-9,16-22 -> courtesy limit lifted; trimmed to shards 2,3 (both done)
#   job 950  %4, shards 4-9,16-22 -> dependency dropped on request; shards 4-7 left running
#   job 907  %4 on h200 normal, shards 23-47 (the two-thirds; unaffected by this)
# This array takes over job 950's *pending* shards (8,9,16-22) with **no throttle** at
# the user's request: concurrency is left to the QoS ceiling instead of a hand-set cap.
# background QoS caps a user at 8 GPUs, so Slurm self-limits -- tasks past the ceiling
# just sit in the queue rather than violating anything. Shards 4-7 stay in job 950.
#   sbatch sam3_gr1_split_array_nolimit.sh
#
