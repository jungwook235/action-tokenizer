#!/bin/bash
#SBATCH --job-name=full_train_v3_dexjoco_dual_arm_recon_ln_bottleneck16_fs
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=h100
#SBATCH --output=out/%j-full_train_v3_dexjoco_dual_arm_recon_ln_bottleneck16_fs.out
#SBATCH --error=out/%j-full_train_v3_dexjoco_dual_arm_recon_ln_bottleneck16_fs.err
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/dexjoco_dual_arm_v3_recon_ln_bn16
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_dexjoco_dual_arm/v3_recon_ln_bn16_fs
TOK_STEP=100000
...
#    --run-name "actlat_v3_dexjoco_dual_arm_recon_ln_bn16" \
#    --num-gpus 2 \
#    --batch-size 1024 \
#    --max-steps $TOK_STEP \
#    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v3_dexjoco_dual_arm_recon_ln_bn16_fs" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
