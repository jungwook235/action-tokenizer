#!/bin/bash
#SBATCH --job-name=full_v2_hand_pred_norecon_mask_fullstate
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-full_v2_hand_pred_norecon_mask_fullstate.out
#SBATCH --error=out/%j-full_v2_hand_pred_norecon_mask_fullstate.err
#SBATCH --comment "full_v2_hand_pred_norecon_mask_fullstate"
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/gr1_100demos_v2_hand_pred_norecon_mask_fullstate
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_gr1_100demos/v2_hand_pred_norecon_mask_fullstate
TOK_STEP=100000

# === Stage 1: Tokenizer Training ===
# Base: v2_hand_pred_norecon_mask (현 SOTA 41.92%)
# 변경: --hand-state-dims 14..25 (12 dim) → 0..28 (29 dim 전체, arms+hands+waist)
# 의도: "hand-specific dexterous" aux → "general future state prediction" aux로 framing.
#        Posttrain OOD 이득 유지/향상 여부 측정.
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_hand_pred_norecon_mask_fullstate" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_hand_pred_norecon_mask_fullstate_gr1_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
