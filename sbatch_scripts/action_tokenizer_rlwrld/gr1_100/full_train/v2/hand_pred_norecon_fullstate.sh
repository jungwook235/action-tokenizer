#!/bin/bash
#SBATCH --job-name=full_v2_hand_pred_norecon_fullstate
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-full_v2_hand_pred_norecon_fullstate.out
#SBATCH --error=out/%j-full_v2_hand_pred_norecon_fullstate.err
#SBATCH --comment "full_v2_hand_pred_norecon_fullstate"
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/gr1_100demos_v2_hand_pred_norecon_fullstate
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_gr1_100demos/v2_hand_pred_norecon_fullstate
TOK_STEP=100000

# === Stage 1: Tokenizer Training ===
# Base: v2_hand_pred_norecon_mask_fullstate (Exp1, AVG 42.00%).
# 변경: masking 관련 4개 인자 제거 (--lambda-mask-recon, --mask-ratio-min/max, --mask-batch-ratio).
# 의도: 29-dim full state + hand-KV framework 에서 masking의 순수 기여도를 단일-axis ablation으로 측정.
# 그 외 arg 전부 identical: hand-KV, 29 dim (arms+hands+waist), hand-tok 2, --no-hand-in-recon, depth 2.
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_hand_pred_norecon_fullstate" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_hand_pred_norecon_fullstate_gr1_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
