#!/bin/bash
#SBATCH --job-name=gr00t_swx_full_v2_recon
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-gr00t_swx_full_v2_recon.out
#SBATCH --error=out/%j-gr00t_swx_full_v2_recon.err
#SBATCH --comment "gr00t_swx_full_v2_recon"
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/swx_100demos_v2_recon
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_swx_100demos/v2_recon
TOK_STEP=100000
...
# v2 grid에서 빠져있던 pure recon baseline (robocasa recon.sh 와 동일 recipe, swx 환경).
...
# action_dim=7 — bridge widowx. mask_recon / state_pred 변종들의 lower-bound 레퍼런스.
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_recon_swx_100demos" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_recon_swx_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
