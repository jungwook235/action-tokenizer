#!/bin/bash
#SBATCH --job-name=rcasa_full_v2_state_pred_full_time
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-rcasa_full_v2_state_pred_full_time.out
#SBATCH --error=out/%j-rcasa_full_v2_state_pred_full_time.err
#SBATCH --comment "rcasa_full_v2_state_pred_full_time"
...
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_robocasa_100demos/v2_state_pred_full_time
TOK_STEP=100000
...
# Robocasa 포팅: GR1 `v2_state_pred_full_time` recipe 를 변경 없이 옮김.
# 구성: time-KV + full state prediction (20 dim), no masking.
...
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_state_pred_full_time_robocasa_100demos" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_state_pred_full_time_robocasa_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
