#!/bin/bash
#SBATCH --job-name=rcasa_full_v2_state_pred_full_time_mask
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-rcasa_full_v2_state_pred_full_time_mask.out
#SBATCH --error=out/%j-rcasa_full_v2_state_pred_full_time_mask.err
#SBATCH --comment "rcasa_full_v2_state_pred_full_time_mask"
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/robocasa_100demos_v2_state_pred_full_time_mask
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_robocasa_100demos/v2_state_pred_full_time_mask
TOK_STEP=100000
...
# Robocasa 포팅: GR1 `v2_state_pred_full_time_mask` recipe 를 변경 없이 옮김.
...
# lambda_mask_hand_pred 는 미사용 (vanilla port).
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_state_pred_full_time_mask_robocasa_100demos" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_state_pred_full_time_mask_robocasa_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
