#!/bin/bash
#SBATCH --job-name=rcasa_full_v2_maskloss
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-rcasa_full_v2_maskloss.out
#SBATCH --error=out/%j-rcasa_full_v2_maskloss.err
#SBATCH --comment "rcasa_full_v2_maskloss"
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/robocasa_100demos_v2_maskloss
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_robocasa_100demos/v2_maskloss
TOK_STEP=100000
...
# Ablation: hand_pred_norecon_mask_fullstate에서 state prediction 축만 제거.
#   - num-hand-tokens 0 (hand token 없이 pure time)
#   - lambda-hand-pred 0 (state pred 비활성, default)
#   - lambda-mask-recon 1.0 (masking 활성)
#   - no state-pred-kv-source / hand-state-dims / hand-pred-future-steps
# masking 단독 기여도 측정용.
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_maskloss_robocasa_100demos" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_maskloss_robocasa_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
