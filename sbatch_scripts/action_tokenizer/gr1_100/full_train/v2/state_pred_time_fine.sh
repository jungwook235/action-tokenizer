#!/bin/bash
#SBATCH --job-name=full_v2_state_pred_time_fine
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-full_v2_state_pred_time_fine.out
#SBATCH --error=out/%j-full_v2_state_pred_time_fine.err
#SBATCH --comment "full_v2_state_pred_time_fine"
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/gr1_100demos_v2_state_pred_time_fine
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_gr1_100demos/v2_state_pred_time_fine
TOK_STEP=100000

# === Stage 1: Tokenizer Training ===
# Exp 2: Fine-grained temporal state prediction (4, 8, 12, 16 steps).
# Baseline v2_state_pred_time과 비교하여 변경: --hand-pred-future-steps (8 16 → 4 8 12 16).
...
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_state_pred_time_fine" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
    ...
    --hand-pred-future-steps 4 8 12 16 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_state_pred_time_fine_gr1_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
