#!/bin/bash
#SBATCH --job-name=full_v2_state_pred_time_long
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-full_v2_state_pred_time_long.out
#SBATCH --error=out/%j-full_v2_state_pred_time_long.err
#SBATCH --comment "full_v2_state_pred_time_long"
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/gr1_100demos_v2_state_pred_time_long
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_gr1_100demos/v2_state_pred_time_long
TOK_STEP=100000

# === Stage 1: Tokenizer Training ===
# Exp 3: Long-horizon state prediction (16, 32 steps; world modeling).
# Baseline v2_state_pred_time과 비교하여 변경: --hand-pred-future-steps (8 16 → 16 32).
# 그 외 모든 arg identical.
# Note: 32-step future는 action chunk(16) 넘어섬. Episode 끝을 넘는 sample은
#       dataset_action_state_pretransform.py line 253에서 traj_len-1로 clip.
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_state_pred_time_long" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_state_pred_time_long_gr1_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
