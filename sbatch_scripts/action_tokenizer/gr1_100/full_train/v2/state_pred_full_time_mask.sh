#!/bin/bash
#SBATCH --job-name=full_v2_state_pred_full_time_mask
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-full_v2_state_pred_full_time_mask.out
#SBATCH --error=out/%j-full_v2_state_pred_full_time_mask.err
#SBATCH --comment "full_v2_state_pred_full_time_mask"
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/gr1_100demos_v2_state_pred_full_time_mask
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_gr1_100demos/v2_state_pred_full_time_mask
TOK_STEP=100000

# === Stage 1: Tokenizer Training ===
# Full 29-dim state prediction (arms+hands+waist) with time-token KV + masking.
# Base: v2_state_pred_full_time (29 dim, KV=time, no mask).
# 변경: --lambda-mask-recon 1.0 / --mask-ratio-min 0.2 / --mask-ratio-max 0.4 / --mask-batch-ratio 0.5 만 추가.
# Mask hparams는 v2_state_pred_time_mask 와 동일.
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_state_pred_full_time_mask" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_state_pred_full_time_mask_gr1_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
