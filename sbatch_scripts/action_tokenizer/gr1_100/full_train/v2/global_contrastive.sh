#!/bin/bash
#SBATCH --job-name=full_actlat_v2_globaltok2_sbatch
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-full_actlat_v2_globaltok2_sbatch.out
#SBATCH --error=out/%j-full_actlat_v2_globaltok2_sbatch.err
#SBATCH --comment "full_actlat_v2_globaltok2_sbatch"

set -x
export PATH="$HOME/.local/bin:$PATH"
export WANDB_API_KEY="66a73856614bc24a07523f3fbee42482fcbeffe3"
export HF_TOKEN="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"

source /sjw_alinlab1/home/jungwook/miniconda3/bin/activate gr00t

CKPT_DIR="checkpoints_action_tokenizer/gr1_100demos_v2_globaltok2"
DATA_DIR=/storage1/sjw_dataset/dataset/robocasa_gr1_tabletop/sim_100demos

# V2 + Global token contrastive learning
# FAST tokenizer로 action → discrete tokens → text encoder → contrastive with global tokens
python scripts/train_action_latent_tokenizer_v2.py \
    --dataset-path $DATA_DIR \
    --output-dir $CKPT_DIR \
    --data-config fourier_gr1_arms_waist \
    --embodiment-tag new_embodiment \
    --run-name "actlat_v2_globaltok2" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps 100000 \
    --save-steps 5000 \
    --num-global-tokens 2 \
    --num-hand-tokens 0 \
    --lambda-recon 1.0 \
    --lambda-global 1 \
    --global-loss-mode contrastive \
    --text-encoder-width 256 \
    --text-encoder-layers 4 \
    --text-encoder-heads 4 \
    --fast-tokenizer-path "physical-intelligence/fast" \
    --fast-vocab-size 2048 \
    --recon-loss-type mse \
    --decoder-mode "self_attention" \
    --wandb-project "action-latent-tokenizer-v2" \
    --eval-steps 1000 \
    --report-to wandb


CKPT_DIR="checkpoints/vla_actlat_fm_gr1_100demos/v2_globaltok2"
DATA_DIR=/storage1/sjw_dataset/dataset/robocasa_gr1_tabletop/sim_100demos
# V2 tokenizer checkpoint 경로를 여기에 입력
TOKENIZER_PATH=/sjw_alinlab1/home/jungwook/Isaac-GR00T/checkpoints_action_tokenizer/gr1_100demos_v2_globaltok2/checkpoint-100000

python scripts/gr00t_finetune_actlat_fm.py \
    --dataset-path $DATA_DIR \
    --output-dir $CKPT_DIR \
    --data-config fourier_gr1_arms_waist_actlat_fm \
    --embodiment-tag new_embodiment \
    --base-model-path "nvidia/GR00T-N1.5-3B" \
    --run-name "actlat_fm_v2_globaltok2_gr1_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
    --eval-steps 1000 \
    --actlat-tokenizer-path $TOKENIZER_PATH \
    --actlat-target-tokens "all" \
    --val-ratio 0.003 \
    --video-backend "decord"