#!/bin/bash
#SBATCH --job-name=full_v2_state_pred_full_time_mask_statemask
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-full_v2_state_pred_full_time_mask_statemask.out
#SBATCH --error=out/%j-full_v2_state_pred_full_time_mask_statemask.err
#SBATCH --comment "full_v2_state_pred_full_time_mask_statemask"
...
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_gr1_100demos/v2_state_pred_full_time_mask_statemask
TOK_STEP=100000
ABS_TOK_CKPT="/sjw_alinlab1/home/jungwook/Isaac-GR00T/$TOK_CKPT_DIR"

# === Stage 1: Tokenizer Training ===
# Base: v2_state_pred_full_time_mask (AVG24=41.75% — GR1 100demos, 2026-04-22).
# 변경 1개: --lambda-mask-hand-pred 1.0 추가.
#   → masked forward 가 생성한 m_time 을 state decoder 의 KV 로도 공급해 새 loss 계산.
#   → "부분 관측 하에서도 action latent 가 state-change 정보를 유지" 하도록 regularization.
# 나머지 인자 전부 v2_state_pred_full_time_mask 와 동일.
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_state_pred_full_time_mask_statemask" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_state_pred_full_time_mask_statemask_gr1_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
