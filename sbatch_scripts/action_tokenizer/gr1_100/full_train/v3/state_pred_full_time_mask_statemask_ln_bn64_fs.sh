#!/bin/bash
#SBATCH --job-name=full_v3_gr1_state_pred_full_time_mask_statemask_ln_bn64_fs
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-full_v3_gr1_state_pred_full_time_mask_statemask_ln_bn64_fs.out
#SBATCH --error=out/%j-full_v3_gr1_state_pred_full_time_mask_statemask_ln_bn64_fs.err
#SBATCH --comment "full_v3_gr1_state_pred_full_time_mask_statemask_ln_bn64_fs"
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/gr1_100demos_v3_state_pred_full_time_mask_statemask_ln_bn64
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_gr1_100demos/v3_state_pred_full_time_mask_statemask_ln_bn64_fs
TOK_STEP=100000
...
# v2/state_pred_full_time_mask_statemask
# (gr1_100demos_v2_state_pred_full_time_mask_statemask) 와 동일한 하이퍼파라미터에
# v3 옵션 두 개만 추가:
...
# Full state pred (29 dims) + masked recon + masked state pred (statemask).
python scripts/train_action_latent_tokenizer_v3.py \
    ...
    --run-name "actlat_v3_gr1_state_pred_full_time_mask_statemask_ln_bn64" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v3_gr1_state_pred_full_time_mask_statemask_ln_bn64_fs_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
