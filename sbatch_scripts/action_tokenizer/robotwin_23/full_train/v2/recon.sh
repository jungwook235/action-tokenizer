#!/bin/bash
#SBATCH --job-name=gr00t_robotwin_23_full_v2_recon
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-gr00t_robotwin_23_full_v2_recon.out
#SBATCH --error=out/%j-gr00t_robotwin_23_full_v2_recon.err
#SBATCH --comment "gr00t_robotwin_23_full_v2_recon"
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/robotwin_23_v2_recon
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_robotwin_23/v2_recon
TOK_STEP=100000
...
# v2 grid 의 pure recon baseline (swx recon.sh 와 동일 recipe, robotwin_23 환경).
...
# action_dim=16 — RoboTwin bimanual franka. mask / state_pred 변종들의 lower-bound 레퍼런스.
python scripts/train_action_latent_tokenizer_v2.py \
    ...
    --run-name "actlat_v2_recon_robotwin_23" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v2_recon_robotwin_23" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
