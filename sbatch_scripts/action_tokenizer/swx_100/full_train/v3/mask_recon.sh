#!/bin/bash
#SBATCH --job-name=gr00t_swx_full_v2_maskloss
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-gr00t_swx_full_v2_maskloss.out
#SBATCH --error=out/%j-gr00t_swx_full_v2_maskloss.err
#SBATCH --comment "gr00t_swx_full_v2_maskloss"

set -x
export PATH="$HOME/.local/bin:$PATH"
export WANDB_API_KEY="66a73856614bc24a07523f3fbee42482fcbeffe3"
export HF_TOKEN="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"

source /sjw_alinlab1/home/jungwook/miniconda3/bin/activate gr00t

DATA_DIR=/storage1/sjw_dataset/dataset/huggingface/lerobot/shared/bridge_orig_lerobot
TOK_CKPT_DIR=checkpoints_action_tokenizer/swx_100demos_v2_maskloss
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_swx_100demos/v2_maskloss
TOK_STEP=100000

# Stage2 VLA wandb project (swx 전용).
export WANDB_PROJECT="gr00t-actlat-fm-swx"

# === Stage 1: Tokenizer Training — Masking-only ablation ===
# Ablation: state prediction 축 제거하고 masking 단독 효과만 측정.
#   - num-hand-tokens 0 (hand token 없이 pure time)
#   - lambda-hand-pred 0 (state pred 비활성, default)
#   - lambda-mask-recon 1.0 (masking 활성)
#   - no state-pred-kv-source / hand-state-dims / hand-pred-future-steps
# action_dim=7 — bridge widowx.
python scripts/train_action_latent_tokenizer_v2.py \
    --dataset-path $DATA_DIR \
    --output-dir $TOK_CKPT_DIR \
    --no-resume \
    --data-config bridge_flare_kty_actlat_fm \
    --embodiment-tag new_embodiment \
    --run-name "actlat_v2_maskloss_swx_100demos" \
    --num-gpus 2 \
    --batch-size 1024 \
    --max-steps $TOK_STEP \
    --save-steps 5000 \
    --num-global-tokens 0 \
    --num-hand-tokens 0 \
    --lambda-recon 1.0 \
    --lambda-mask-recon 1.0 \
    --mask-ratio-min 0.2 \
    --mask-ratio-max 0.4 \
    --mask-batch-ratio 0.5 \
    --recon-loss-type mse \
    --decoder-mode self_attention \
    --wandb-project "action-latent-tokenizer-v2-swx" \
    --eval-steps 1000 \
    --report-to wandb

# === Stage 2: VLA Training ===
# num-hand-tokens=0 이므로 target_tokens="all"은 실질 time 16개만 denoise.
python scripts/gr00t_finetune_actlat_fm.py \
    --dataset-path $DATA_DIR \
    --output-dir $VLA_CKPT_DIR \
    --data-config bridge_flare_kty_actlat_fm \
    --embodiment-tag new_embodiment \
    --base-model-path "nvidia/GR00T-N1.5-3B" \
    --run-name "actlat_fm_v2_maskloss_swx_100demos" \
    --num-gpus 2 \
    --batch-size 32 \
    --max-steps 60000 \
    --save-steps 10000 \
    --eval-steps 1000 \
    --actlat-tokenizer-path $TOK_CKPT_DIR/checkpoint-$TOK_STEP \
    --actlat-target-tokens "all" \
    --val-ratio 0.003 \
    --video-backend "torchvision_av"
