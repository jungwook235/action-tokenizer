#!/bin/bash
#SBATCH --job-name=full_v4_gr1_recon_vggt_bn64
#SBATCH --partition=gpu
#SBATCH --gres=gpu:b200:4
#SBATCH --nodes=1
#SBATCH --time=72:00:00
#SBATCH --output=/NHNHOME/data/wook/action-tokenizer/slurm/logs/full_v4_gr1_recon_vggt_bn64_%j.out
#SBATCH --error=/NHNHOME/data/wook/action-tokenizer/slurm/logs/full_v4_gr1_recon_vggt_bn64_%j.err
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/gr1_100demos_v4_recon_vggt_bn64
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_gr1_100demos/v4_recon_vggt_bn64_fs
TOK_STEP=100000
ABS_TOK_CKPT="/NHNHOME/data/wook/action-tokenizer/$TOK_CKPT_DIR"

# === Stage 1: V4 Tokenizer Training — RLA-VGGT hybrid (robocasa) ===
...
python scripts/train_action_latent_tokenizer_v4.py \
    ...
    --run-name "actlat_v4_gr1_recon_vggt_bn64_100demos" \
    --num-gpus 4 \
    --batch-size 128 \
    --max-steps $TOK_STEP \
    --save-steps 10000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v4_gr1_recon_vggt_bn64_fs_100demos" \
    --num-gpus 4 \
    --batch-size 16 \
    --max-steps 60000 \
    --save-steps 5000 \
