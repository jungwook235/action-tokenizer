#!/bin/bash
#SBATCH --job-name=full_v4_gr1_recon_vggt_bn64_l1_mse
#SBATCH --partition=gpu
#SBATCH --gres=gpu:b200:4
#SBATCH --nodes=1
#SBATCH --time=72:00:00
#SBATCH --output=/NHNHOME/data/wook/action-tokenizer/slurm/logs/full_v4_gr1_recon_vggt_bn64_l1_mse_%j.out
#SBATCH --error=/NHNHOME/data/wook/action-tokenizer/slurm/logs/full_v4_gr1_recon_vggt_bn64_l1_mse_%j.err
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/gr1_100demos_v4_recon_vggt_bn64_l1_mse
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_gr1_100demos/v4_recon_vggt_bn64_l1_mse_fs
TOK_STEP=100000
ABS_TOK_CKPT="/NHNHOME/data/wook/action-tokenizer/$TOK_CKPT_DIR"

# === Stage 1: V4 Tokenizer Training — RLA-VGGT hybrid, L1 reconstruction loss ===
# Same as recon_vggt_bn64.sh, except uses L1 reconstruction loss (--recon-loss-type l1).
#
...
python scripts/train_action_latent_tokenizer_v4.py \
    ...
    --run-name "actlat_v4_gr1_recon_vggt_bn64_l1_mse_100demos" \
    --num-gpus 4 \
    --batch-size 64 \
    --max-steps $TOK_STEP \
    --save-steps 10000 \
...
# The wrapper auto-detects the VGGT feature source from the tokenizer checkpoint
# (recorded markers) and rebuilds the matching frozen VGGT extractor; frames flow
# through unchanged.
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v4_gr1_recon_vggt_bn64_l1_mse_fs_100demos" \
    --num-gpus 4 \
    --batch-size 16 \
    --max-steps 60000 \
    --save-steps 5000 \
