#!/bin/bash
#SBATCH --job-name=full_v4_gr1_recon_dino_bn32_l1_mse_naiveln
#SBATCH --partition=gpu
#SBATCH --gres=gpu:b200:4
#SBATCH --nodes=1
#SBATCH --time=72:00:00
#SBATCH --output=/NHNHOME/data/wook/action-tokenizer/slurm/logs/full_v4_gr1_recon_dino_bn32_l1_mse_naiveln_%j.out
#SBATCH --error=/NHNHOME/data/wook/action-tokenizer/slurm/logs/full_v4_gr1_recon_dino_bn32_l1_mse_naiveln_%j.err
...
TOK_CKPT_DIR=checkpoints_action_tokenizer/gr1_100demos_v4_recon_dino_bn32_l1_mse_naiveln
VLA_CKPT_DIR=checkpoints/vla_actlat_fm_gr1_100demos/v4_recon_dino_bn32_l1_mse_naiveln_fs
TOK_STEP=100000
ABS_TOK_CKPT="/NHNHOME/data/wook/action-tokenizer/$TOK_CKPT_DIR"

# === Stage 1: V4 Tokenizer Training — RLA-DINO hybrid, NAIVE final LayerNorm ===
# Same as recon_dino_bn64_l1_mse.sh, except DINO patch features use a NON-AFFINE
# final LayerNorm (--dino-final-norm naive): the final LN's learned γ/β are dropped
# and the last encoder hidden state is normalized with a plain (x-mean)/std. The
# Stage-2 wrapper auto-detects this from a checkpoint marker and matches it.
#
...
python scripts/train_action_latent_tokenizer_v4.py \
    ...
    --run-name "actlat_v4_gr1_recon_dino_bn32_l1_mse_naiveln_100demos" \
    --num-gpus 4 \
    --batch-size 128 \
    --max-steps $TOK_STEP \
    --save-steps 10000 \
...
python scripts/gr00t_finetune_actlat_fm.py \
    ...
    --run-name "actlat_fm_v4_gr1_recon_dino_bn32_l1_mse_naiveln_fs_100demos" \
    --num-gpus 4 \
    --batch-size 16 \
    --max-steps 60000 \
    --save-steps 5000 \
