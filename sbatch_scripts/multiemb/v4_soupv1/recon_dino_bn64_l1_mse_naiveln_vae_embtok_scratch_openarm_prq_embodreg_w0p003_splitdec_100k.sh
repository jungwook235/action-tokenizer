#!/bin/bash
#SBATCH --job-name=scratch_v4_openarm_prq_embodreg_vicreg_w0p003_splitdec_recon_dino_bn64_l1_mse_naiveln_vae_embtok_100k
#SBATCH --nodes=1
#SBATCH --gpus=8
#SBATCH --partition=sjw_alinlab,h100
#SBATCH --wckey=project-short-name:sub_human
#SBATCH --output=out-multiemb/%j-scratch_v4_openarm_prq_embodreg_w0p003_splitdec_100k.out
#SBATCH --error=out-multiemb/%j-scratch_v4_openarm_prq_embodreg_w0p003_splitdec_100k.err
...
#MODEL_OUTPUT_DIR=/rlwrld-unified-checkpoints/jungwook/action_tokenizer/checkpoints_action_tokenizer sbatch '/sjw_alinlab1/home/jungwook/action_tokenizer/sbatch_scripts/multiemb/v4_soupv1/recon_dino_bn64_l1_mse_naiveln_vae_embtok_scratch_openarm_prq_embodreg_w0p003_splitdec_100k.sh'
...
#     representation is being formed. Hence the w0p03 sibling (10x) as the other bracket.
...
TOK_CKPT_DIR=$MODEL_OUTPUT_DIR/checkpoints_action_tokenizer/joint_openarm_prq_v4_scratch_recon_dino_bn64_l1_mse_naiveln_vae_embtok_embodreg_vicreg_w0p003_splitdec_100k
FT_STEP=100000
...
python scripts/train_action_latent_tokenizer_v4_multiemb.py \
    ...
    --run-name "actlat_v4_scratch_openarm_prq_embodreg_vicreg_w0p003_splitdec_recon_dino_bn64_l1_mse_naiveln_vae_embtok_100k" \
    --num-gpus 8 \
    --batch-size 64 \
    --max-steps $FT_STEP \
    --save-steps 5000 \
...
# Sibling: ..._scratch_openarm_prq_embodreg_w0p03_splitdec_100k.sh is this file with
# --embod-reg-weight 0.03 (10x) and its own job-name / run-name / output-dir / out-err
# names. Nothing else differs, so the pair is a single-variable weight sweep.
