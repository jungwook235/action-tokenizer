#!/bin/bash
#SBATCH --job-name=scratch_v4_openarm_prq_embodreg_vicreg_w0p03_splitdec_recon_dino_bn64_l1_mse_naiveln_vae_embtok_400k
#SBATCH --nodes=1
#SBATCH --gpus=8
#SBATCH --partition=sjw_alinlab,h100
#SBATCH --wckey=project-short-name:sub_human
#SBATCH --output=out-multiemb/%j-scratch_v4_openarm_prq_embodreg_w0p03_splitdec_400k.out
#SBATCH --error=out-multiemb/%j-scratch_v4_openarm_prq_embodreg_w0p03_splitdec_400k.err
...
#MODEL_OUTPUT_DIR=/rlwrld-unified-checkpoints/jungwook/action_tokenizer/checkpoints_action_tokenizer sbatch '/sjw_alinlab1/home/jungwook/action_tokenizer/sbatch_scripts/multiemb/v4_soupv1/recon_dino_bn64_l1_mse_naiveln_vae_embtok_scratch_openarm_prq_embodreg_w0p03_splitdec_400k.sh'
...
#     is being formed. THIS SCRIPT IS THE 10x RESPONSE to that: weight 0.03 instead of
#     0.003, to restore some of the relative pull during early training. 0.03 is NOT a
#     measured value either -- it is one decade up from a value whose basis no longer
#     applies, chosen to bracket the sweep. If loss_recon is still badly elevated against
#     the w0p003 sibling at 5k-10k steps, the weight is too high, not the hinge.
...
TOK_CKPT_DIR=$MODEL_OUTPUT_DIR/checkpoints_action_tokenizer/joint_openarm_prq_v4_scratch_recon_dino_bn64_l1_mse_naiveln_vae_embtok_embodreg_vicreg_w0p03_splitdec_400k
TOK_STEP=400000
...
python scripts/train_action_latent_tokenizer_v4_multiemb.py \
    ...
    --run-name "actlat_v4_scratch_openarm_prq_embodreg_vicreg_w0p03_splitdec_recon_dino_bn64_l1_mse_naiveln_vae_embtok_400k" \
    --num-gpus 8 \
    --batch-size 64 \
    --max-steps $TOK_STEP \
    --save-steps 10000 \
...
# Sibling: ..._scratch_openarm_prq_embodreg_w0p003_splitdec_400k.sh is this file with
# --embod-reg-weight 0.003 (the EXP-0010 value) and its own run-name / output-dir /
# out-err names. Nothing else differs, so the pair is a single-variable weight sweep.
