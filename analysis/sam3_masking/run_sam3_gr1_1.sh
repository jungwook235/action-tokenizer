#!/bin/bash
# SAM3 GR-1 masking — job 1 of 4. Owns tasks 0-5 of the 24 sorted
# gr1_unified.* dirs and runs them on this pod's 4 GPUs (episode j of a task -> GPU j%4).
#
# Paired with the k8s Job `jungwook-data-sam3-gr1-1`, whose container args call:
#   bash /data/jungwook/action-tokenizer/analysis/sam3_masking/run_sam3_gr1_1.sh
#
...
#   ./run_sam3_gr1_1.sh --limit 1        # smoke test: 1 episode per GPU shard
set -uo pipefail

export SAM3_JOB_IDX=0     # 0-based; NOT the yaml's JOB_ID (that is a name string)
