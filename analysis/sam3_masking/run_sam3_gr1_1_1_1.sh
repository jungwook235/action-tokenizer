#!/bin/bash
# SAM3 GR-1 masking — job 1-1-1: run_sam3_gr1_1_1.sh split onto a 2-GPU pod.
# Same 3 tasks (dirs 0-2 of the 24 sorted gr1_unified.*) as job 1-1, but this pod
# only runs the FIRST half of each task's episodes: shards 0-1 of 4.
# Its sibling run_sam3_gr1_1_1_2.sh runs shards 2-3 — both must run to finish
# these 3 tasks.
#
# Paired with a k8s Job requesting 2 GPUs, whose container args call:
#   bash /data/jungwook/action-tokenizer/analysis/sam3_masking/run_sam3_gr1_1_1_1.sh
#
...
export SAM3_JOB_IDX=0     # 0-based over the 8 task slices; NOT the yaml's JOB_ID
export SAM3_JOB_COUNT=8
export TASKS_PER_JOB=3
export SHARD_TOTAL=4      # episode split is still 4-way …
export SHARD_BASE=0       # … this pod takes shards 0-1
