#!/bin/bash
# SAM3 GR-1 masking — job 1-1: the 1st half of the original job 1.
# Owns tasks 0-2 of the 24 sorted gr1_unified.* dirs (3 tasks) and runs them on
# this pod's 4 GPUs (episode j of a task -> GPU j%4).
#
# Paired with the k8s Job `jungwook-data-sam3-gr1-1-1`, whose container args call:
#   bash /data/jungwook/action-tokenizer/analysis/sam3_masking/run_sam3_gr1_1_1.sh
#
...
#   ./run_sam3_gr1_1_1.sh --limit 1     # smoke test: 1 episode per GPU shard
set -uo pipefail

export SAM3_JOB_IDX=0     # 0-based over the 8 half-jobs; NOT the yaml's JOB_ID
